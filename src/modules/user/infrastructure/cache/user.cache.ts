...
export class UserCache {
  ...
  async setUser(user: User): Promise<void> {
    ...
    await Promise.all([
      ...
      this.cache.set(this.USER_BY_EMAIL_KEY(user.email), userJson, this.USER_TTL_SECONDS * 1000),
      user.phoneNumber && this.cache.set(this.USER_BY_PHONE_KEY(user.phoneNumber), userJson, this.USER_TTL_SECONDS * 1000),
    ].filter(Boolean));
  }
